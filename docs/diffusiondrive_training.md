...
- 推理侧 predicted-speed controller 已支持 `DIFFUSIONDRIVE_USE_SPEED_HEAD=1`，用模型 `target_speed_logits` 的期望速度替代空间 PID 的 longitudinal desired speed；默认 `0` 保持空间 PID fallback。闭环 debug 会打印 `speed_head_class / speed_head_desired / speed_head_brake_prob`。当前 direct controller 已在 `C1_speedhead` route 00 暴露 class-0 全刹锁死问题，正式评测不要继续启用；后续应改成 gated speed cap / brake gate。
