...
- 从环境变量读取：
  ...
  - `DIFFUSIONDRIVE_SPEED_HEAD_BRAKE_THRESHOLD`
  ...
  - `DIFFUSIONDRIVE_STUCK_THRESHOLD`
...
`baseline-condition-v1` 仍不恢复旧 `extra_sensors`。当前已保留 `status_token=command_one_hot(6)+speed(1)`，并新增独立 `route_condition_token=target_point(2)+target_point_next(2)`；不要把 route target 简单塞进 11 维 flat status。该方案改变模型接口，需要 full retrain。
...
- 可选：`DIFFUSIONDRIVE_USE_SPEED_HEAD=1` 时读取 `outputs['target_speed_logits']` 做 predicted-speed longitudinal controller
  - 默认 `DIFFUSIONDRIVE_SPEED_HEAD_UNCERTAINTY_WEIGHT=1`，与 syb 接法一致：class-0 概率超过 `DIFFUSIONDRIVE_SPEED_HEAD_BRAKE_THRESHOLD=0.9` 才强制目标速度为 0，否则使用 `sum(prob * target_speeds)`；设为 `0` 切回 argmax 诊断。
