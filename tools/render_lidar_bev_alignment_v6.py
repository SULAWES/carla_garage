...
and produces:
  ...
  - optional coarse-to-fine local dx/dy/yaw refinement around the measurement-based alignment
  - optional score ROI so evaluation/refinement focuses on a stable near-field static region
  - a JSON summary
...
Examples:
  python tools/render_lidar_bev_alignment_v6.py \
...
class RefinementConfig:
    enabled: bool = True
    coarse_xy_range: float = 3.0
    coarse_xy_step: float = 0.25
    coarse_yaw_range_deg: float = 0.0
    coarse_yaw_step_deg: float = 1.0
    fine_xy_range: float = 0.4
    fine_xy_step: float = 0.05
    fine_yaw_range_deg: float = 0.0
    fine_yaw_step_deg: float = 0.25
    fallback_if_worse: bool = True


@dataclass(frozen=True)
class ScoreRoiConfig:
    enabled: bool = True
    min_x: float = 0.0
    max_x: float = 24.0
    min_y: float = -12.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--disable-refinement",
        action="store_true",
        help="Disable coarse-to-fine dx/dy/yaw refinement after the measurement-based alignment.",
    )
    parser.add_argument(
        "--coarse-refine-xy-range",
        type=float,
        default=3.0,
        help="Coarse residual search range in meters for dx/dy refinement.",
    )
    parser.add_argument(
        "--coarse-refine-xy-step",
        type=float,
        default=0.25,
        help="Coarse residual search step in meters for dx/dy refinement.",
    )
    parser.add_argument(
        "--coarse-refine-yaw-range-deg",
        type=float,
        default=0.0,
        help="Coarse residual search range in degrees for yaw refinement.",
    )
    parser.add_argument(
        "--coarse-refine-yaw-step-deg",
        type=float,
        default=1.0,
        help="Coarse residual search step in degrees for yaw refinement.",
    )
    parser.add_argument(
        "--fine-refine-xy-range",
        type=float,
        default=0.4,
        help="Fine residual search range in meters around the coarse optimum.",
    )
    parser.add_argument(
        "--fine-refine-xy-step",
        type=float,
        default=0.05,
        help="Fine residual search step in meters around the coarse optimum.",
    )
    parser.add_argument(
        "--fine-refine-yaw-range-deg",
        type=float,
        default=0.0,
        help="Fine residual search range in degrees around the coarse optimum.",
    )
    parser.add_argument(
        "--fine-refine-yaw-step-deg",
        type=float,
        default=0.25,
        help="Fine residual search step in degrees around the coarse optimum.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--disable-score-roi",
        action="store_true",
        help="Disable the near-field ROI restriction for masked IoU and refinement scoring.",
    )
    parser.add_argument("--score-roi-min-x", type=float, default=0.0)
    parser.add_argument("--score-roi-max-x", type=float, default=24.0)
    parser.add_argument("--score-roi-min-y", type=float, default=-12.0)
    parser.add_argument("--score-roi-max-y", type=float, default=12.0)
    ...
    if args.coarse_refine_xy_range < 0 or args.coarse_refine_xy_step <= 0:
        parser.error("--coarse-refine-xy-range must be >= 0 and --coarse-refine-xy-step must be > 0.")
    if args.coarse_refine_yaw_range_deg < 0 or args.coarse_refine_yaw_step_deg <= 0:
        parser.error("--coarse-refine-yaw-range-deg must be >= 0 and --coarse-refine-yaw-step-deg must be > 0.")
    if args.fine_refine_xy_range < 0 or args.fine_refine_xy_step <= 0:
        parser.error("--fine-refine-xy-range must be >= 0 and --fine-refine-xy-step must be > 0.")
    if args.fine_refine_yaw_range_deg < 0 or args.fine_refine_yaw_step_deg <= 0:
        parser.error("--fine-refine-yaw-range-deg must be >= 0 and --fine-refine-yaw-step-deg must be > 0.")
    if not (args.score_roi_min_x < args.score_roi_max_x and args.score_roi_min_y < args.score_roi_max_y):
        parser.error("Score ROI bounds must satisfy min < max for both x and y.")
    return args
...
def score_roi_mask(shape: tuple[int, int], cfg: BevConfig, roi_cfg: ScoreRoiConfig) -> np.ndarray:
    mask = np.zeros(shape, dtype=bool)
    ...
    ppm = cfg.pixels_per_meter
    col_min = max(int(np.floor((roi_cfg.min_x - cfg.min_x) * ppm)), 0)
    col_max = min(int(np.ceil((roi_cfg.max_x - cfg.min_x) * ppm)), shape[1])
    row_min = max(int(np.floor((roi_cfg.min_y - cfg.min_y) * ppm)), 0)
    row_max = min(int(np.ceil((roi_cfg.max_y - cfg.min_y) * ppm)), shape[0])
    if row_min < row_max and col_min < col_max:
        mask[row_min:row_max, col_min:col_max] = True
    return mask


def masked_occupancy_iou(a: np.ndarray, b: np.ndarray, exclude_mask: np.ndarray) -> float:
    if exclude_mask.shape != a.shape or exclude_mask.shape != b.shape:
        raise ValueError("Mask shape must match occupancy grids.")
    a_occ = occupancy(a) & ~exclude_mask
    b_occ = occupancy(b) & ~exclude_mask
...
def evaluate_alignment(
    ...
    roi_include_mask: np.ndarray,
) -> dict[str, object]:
    bev = lidar_to_histogram_features(lidar, cfg)
    channel_index = 1 if cfg.use_ground_plane else 0
    channel = bev[channel_index]
    dynamic_actor_mask = dynamic_mask(boxes, channel.shape, cfg, dynamic_classes)
    exclude_mask = current_exclude_mask | dynamic_actor_mask | ~roi_include_mask
    iou = masked_occupancy_iou(current_channel, channel, exclude_mask)
    return {
        "iou": iou,
        "channel": channel,
...
def compose_residual_transform(
    ...
    rotation_a = np.array(
        [
            [np.cos(yaw_a), -np.sin(yaw_a), 0.0],
            [np.sin(yaw_a), np.cos(yaw_a), 0.0],
            [0.0, 0.0, 1.0],
        ],
        dtype=np.float32,
    )
    translation_total = translation_a + rotation_a @ translation_b
    yaw_total = normalize_angle(yaw_a + yaw_b)
    return translation_total.astype(np.float32), yaw_total
...
def search_refinement_stage(
    ...
    base_eval = evaluate_alignment(
        lidar=base_lidar,
        boxes=base_boxes,
        current_channel=current_channel,
        current_exclude_mask=current_exclude_mask,
        cfg=cfg,
        dynamic_classes=dynamic_classes,
        roi_include_mask=roi_include_mask,
    )
    best = {
        "iou": base_eval["iou"],
        ...
        "exclude_mask": base_eval["exclude_mask"],
        "searched_candidates": 1,
        "cost": 0.0,
    }

    dx_values = refinement_values(xy_range, xy_step)
    dy_values = refinement_values(xy_range, xy_step)
    yaw_values_deg = refinement_values(yaw_range_deg, yaw_step_deg)
    ...
    for dx in dx_values:
        for dy in dy_values:
            ...
            for yaw_deg in yaw_values_deg:
                ...
                candidate_lidar = algin_lidar(base_lidar, translation, yaw)
                candidate_boxes = apply_transform_to_boxes(base_boxes, translation, yaw)
                candidate_eval = evaluate_alignment(
                ...
                if candidate_eval["iou"] > best["iou"] + 1e-9 or (
                    abs(candidate_eval["iou"] - best["iou"]) <= 1e-9 and candidate_cost < best["cost"]
                ):
                    best = {
                        ...
                        "exclude_mask": candidate_eval["exclude_mask"],
...
def refine_alignment(
    ...
    initial_eval = evaluate_alignment(
        lidar=aligned_lidar,
        boxes=aligned_boxes,
        current_channel=current_channel,
        current_exclude_mask=current_exclude_mask,
    ...
    best = {
        ...
        "exclude_mask": initial_eval["exclude_mask"],
        "searched_candidates": 1,
        "used_fallback": False,
        "cost": 0.0,
        "coarse_translation": np.zeros(3, dtype=np.float32),
        "coarse_yaw": 0.0,
        "coarse_iou": initial_iou,
        "coarse_candidates": 1,
        "fine_translation": np.zeros(3, dtype=np.float32),
        "fine_yaw": 0.0,
    ...
    total_translation, total_yaw = compose_residual_transform(
    ...
    best = {
        ...
        "exclude_mask": fine["exclude_mask"],
...
def render_for_history(
    ...
    roi_include_mask = score_roi_mask(current_channel.shape, cfg, roi_cfg)
    ...
    current_exclude_mask = current_dynamic_mask | ~roi_include_mask
    before_exclude_mask = current_exclude_mask | history_dynamic_mask
    initial_after_exclude_mask = current_exclude_mask | aligned_history_dynamic_mask
    ...
    before_iou = masked_occupancy_iou(current_channel, history_channel, before_exclude_mask)
    initial_after_iou = masked_occupancy_iou(current_channel, aligned_channel, initial_after_exclude_mask)
    ...
    refined_exclude_mask = refinement["exclude_mask"]
    after_iou_raw = occupancy_iou(current_channel, refined_channel)
    after_iou = refinement["iou"]
    visualization_mask = before_exclude_mask | refined_exclude_mask

    current_channel_vis = np.where(visualization_mask, 0.0, current_channel)
    current_before_vis = np.where(before_exclude_mask, 0.0, current_channel)
    current_after_vis = np.where(refined_exclude_mask, 0.0, current_channel)
    history_channel_vis = np.where(before_exclude_mask, 0.0, history_channel)
    aligned_channel_vis = np.where(refined_exclude_mask, 0.0, refined_channel)
    ...
    return {
        ...
        "roi_pixels": int(roi_include_mask.sum()),
        "before_masked_pixels": int(before_exclude_mask.sum()),
        "initial_after_masked_pixels": int(initial_after_exclude_mask.sum()),
        "after_masked_pixels": int(refined_exclude_mask.sum()),
    ...
    for current_frame in frames:
        ...
        for index, history_offset in enumerate(history_offsets):
            ...
            print(
                ...
                f"coarse=({result['coarse_refinement_dx']:.2f},{result['coarse_refinement_dy']:.2f},{result['coarse_refinement_yaw_deg']:.2f}deg) "
                f"fine=({result['fine_refinement_dx']:.2f},{result['fine_refinement_dy']:.2f},{result['fine_refinement_yaw_deg']:.2f}deg) "
                f"total=({result['refinement_dx']:.2f},{result['refinement_dy']:.2f},{result['refinement_yaw_deg']:.2f}deg) "
...
def main() -> int:
    ...
    roi_cfg = ScoreRoiConfig(
        enabled=not args.disable_score_roi,
        min_x=args.score_roi_min_x,
        max_x=args.score_roi_max_x,
        min_y=args.score_roi_min_y,
        max_y=args.score_roi_max_y,
    )
    refinement_cfg = RefinementConfig(
    ...
    summary = {
        ...
        "config": {
            ...
            "score_roi_enabled": roi_cfg.enabled,
            "score_roi_min_x": roi_cfg.min_x,
            "score_roi_max_x": roi_cfg.max_x,
            "score_roi_min_y": roi_cfg.min_y,
            "score_roi_max_y": roi_cfg.max_y,
            "refinement_enabled": refinement_cfg.enabled,
