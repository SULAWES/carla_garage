...
and produces:
  ...
  - optional common-support scoring mask derived from LiDAR occupancy
  - a JSON summary
...
Examples:
  python tools/render_lidar_bev_alignment_v7.py \
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--disable-common-support-mask",
        action="store_true",
        help="Disable data-driven common-support masking based on LiDAR occupancy.",
    )
    parser.add_argument(
        "--support-dilation-pixels",
        type=int,
        default=2,
        help="Binary dilation radius in pixels used to build the common-support mask.",
    )
    ...
    if args.support_dilation_pixels < 0:
        parser.error("--support-dilation-pixels must be >= 0.")
    return args
...
def dilate_binary(mask: np.ndarray, radius: int) -> np.ndarray:
    ...
    for dy in range(-radius, radius + 1):
        for dx in range(-radius, radius + 1):
            ...
            row_start = radius + dy
            col_start = radius + dx
            dilated |= padded[row_start : row_start + mask.shape[0], col_start : col_start + mask.shape[1]]
    return dilated


def common_support_include_mask(
    current_channel: np.ndarray,
    compare_channel: np.ndarray,
    support_cfg: SupportConfig,
) -> np.ndarray:
    if not support_cfg.enabled:
        return np.ones_like(current_channel, dtype=bool)
    current_support = dilate_binary(occupancy(current_channel), support_cfg.dilation_pixels)
    compare_support = dilate_binary(occupancy(compare_channel), support_cfg.dilation_pixels)
    return current_support & compare_support
...
def evaluate_alignment(
    ...
    support_include = common_support_include_mask(current_channel, channel, support_cfg)
    support_pixels = int(support_include.sum())
    eval_mask = current_dynamic_mask | dynamic_actor_mask | ~support_include
    iou = 0.0 if support_pixels == 0 else masked_occupancy_iou(current_channel, channel, eval_mask)
    return {
        "iou": iou,
        "channel": channel,
...
def render_for_history(
    ...
    return {
        ...
        "support_pixels": int(refinement["support_pixels"]),
    ...
    for current_frame in frames:
        ...
        for index, history_offset in enumerate(history_offsets):
            ...
            print(
                ...
                f"support_pixels={result['support_pixels']} "
